...
import { ConfirmOfferModal, CreateOfferStepOne, CreateOfferStepThree, CreateOfferStepTwo } from 'src/containers';
import {
    useDocumentTitle,
...
export const CreateP2POfferScreen: FC = (): ReactElement => {
    ...
    useEffect(() => {
        setAmountPrecision(
            currencies.find((c) => c.id === asset?.id.toLowerCase())?.precision || DEFAULT_CCY_PRECISION,
        );
    }, [asset, currencies]);
    ...
    const updatePaymentMethod = React.useCallback(
        (value: UserPaymentMethod, index: number) => {
    ...
    const popPaymentMethod = React.useCallback(
        (index: number) => {
    ...
    return (
        <div className="pg-create-offer pg-container">
            ...
            <div className="pg-create-offer-box">
                ...
                <div className="pg-create-offer__content">{content}</div>
            </div>
            {confirmOfferModal}
        </div>
    );
