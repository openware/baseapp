...
class CanCanService extends React.Component<Props> {
    public static check = (action: string, target: string | TargetByField, abilities: AbilitiesInterface, loading: boolean) => {
        if (!loading && abilities
            && ((typeof target === 'string' && (CanCanService.checkAbilityByAction(action, target, abilities) || CanCanService.checkAbilityByAction('read', target, abilities)))
            || (typeof target === 'object' && (CanCanService.checkFieldByAction(action, target, abilities) || (CanCanService.checkFieldByAction('read', target, abilities))))
            || !target)
        ) {
            return true;
        }
    ...
    public static checkFieldByAction = (action: string, target: TargetByField, abilities: AbilitiesInterface) => (
            ...
            (target.field && abilities[action].filter(item => item instanceof Object && item[target.name] && item[target.name].includes(target.field)).length)
