...
import { DEFAULT_CCY_PRECISION, DEFAULT_FIAT_PRECISION, DEFAULT_TABLE_PAGE_LIMIT, HOST_URL } from 'src/constants';
...
const P2POffers: FC<Props> = (props: Props): ReactElement => {
    ...
    const retrieveData = useCallback(
        (amountPrecision: number, pricePrecision: number) =>
            list.map((item) => {
                const { id, price, user, available_amount, min_order_amount, max_order_amount, payment_methods } = item;
                ...
                return [
                    <div className="advertisers" key={id}>
                        ...
                        <div className="font-small ml-small">
                            ...
                            <div className="sec-row">
                                <span className="font-small secondary">
                                    {user?.offers_count}&nbsp;
                                    {intl.formatMessage({ id: 'page.body.p2p.table.offers' })}
                                </span>
                                <span className="font-small secondary ml-24">
                                    {user?.success_rate ? `${Decimal.format(+user?.success_rate * 100, 2)}` : `0.00`}%{' '}
                    ...
                    <div className="limit" key={id}>
                        <div className="limit-col">
                            <span className="font-small secondary">
                                {intl.formatMessage({ id: 'page.body.p2p.table.available' })}
                            </span>
                            <span className="font-small secondary sec-row">
                                {intl.formatMessage({ id: 'page.body.p2p.table.limit' })}
                            </span>
                        </div>
                        <div className="limit-col">
                            <span className="font-big ml-small">
                                {Decimal.format(+available_amount, amountPrecision, ',')} {item.base?.toUpperCase()}
                            </span>
                            <span className="font-big ml-small sec-row">
                                {Decimal.format(+min_order_amount * +price, pricePrecision, ',')} -{' '}
                                {Decimal.format(+max_order_amount * +price, pricePrecision, ',')}{' '}
                    ...
                    <div className="trade" key={id}>
                        <Button onClick={() => props.onClickTrade(item)} size="lg" variant={side}>
