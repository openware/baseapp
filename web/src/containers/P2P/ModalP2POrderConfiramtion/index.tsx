...
const P2POrderConfirmModal: FC<Props> = (props: Props): ReactElement => {
        ...
        () => (
            <React.Fragment>
                <span className="cr-modal__container-header-title">
                    {translate(`page.body.p2p.order.modal.header.${side}Order`)}
                </span>
                    ...
                    <div className="detail">
                        <div className="detail-block">
                            <div className="detail-block-label">
                                {translate('page.body.p2p.order.modal.body.amount')}
                            </div>
                            <div className="detail-block-value">
                                {order
                                    ? Decimal.format(+order.amount * +order?.offer?.price, precissionQuote, ',')
                                    : '-'}
                        ...
                        <div className="detail-block">
                            <div className="detail-block-label">
                                {translate('page.body.p2p.order.modal.body.price')}
                            </div>
                            <div className="detail-block-value">
                                {order ? Decimal.format(order?.offer?.price, precissionQuote, ',') : '-'}
                                &nbsp;{order?.offer?.quote?.toUpperCase()}
                            </div>
                        </div>
                        <div className="detail-block">
                            <div className="detail-block-label">
                                {translate('page.body.p2p.order.modal.body.quantity')}
                            </div>
                            <div className="detail-block-value">
                                {order ? Decimal.format(order.amount, precissionBase, ',') : '-'}
    ...
    const footer = React.useCallback(
        () => (
            <Button onClick={props.closeModal} size="lg" variant="primary">
