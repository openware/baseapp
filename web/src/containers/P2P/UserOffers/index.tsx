...
const P2PUserOffers: FC = (): ReactElement => {
    ...
    const firstElemIndex = useSelector((state: RootState) =>
        selectP2PUserOffersFirstElemIndex(state, DEFAULT_TABLE_PAGE_LIMIT),
    );
    const lastElemIndex = useSelector((state: RootState) =>
        selectP2PUserOffersLastElemIndex(state, DEFAULT_TABLE_PAGE_LIMIT),
    );
    const nextPageExists = useSelector((state: RootState) =>
        selectP2PUserOffersNextPageExists(state, DEFAULT_TABLE_PAGE_LIMIT),
    );
    ...
    const retrieveData = useCallback(
        () =>
            !list.length
                ? [[]]
                : list.map((item) => {
                      const { id, side, created_at, price, origin_amount, quote, base, state, orders_count } = item;

                      const amountPrecision = wallets.find((w) => w.currency === base)?.fixed || DEFAULT_CCY_PRECISION;
                      const pricePrecision =
                          wallets.find((obj) => obj.currency === quote)?.fixed || DEFAULT_FIAT_PRECISION;

                      return [
                          localeDate(created_at, 'shortDate'),
                          <span style={{ color: setOfferStatusColor(side) }} key={id}>
                              {translate(`page.body.p2p.my.offers.table.side.${side}`)}
                          </span>,
                          `${base?.toUpperCase()}/${quote?.toUpperCase()}`,
                          <span key={id}>
                              {Decimal.format(origin_amount, amountPrecision, ',')} {base?.toUpperCase()}
                          </span>,
                          <span key={id}>
                              {Decimal.format(price, pricePrecision, ',')} {quote?.toUpperCase()}
                          </span>,
                          <span style={{ color: setOfferStatusColor(state) }} className="text-capitalize" key={id}>
                              {translate(`page.body.p2p.my.offers.${state}`)}
                          </span>,
