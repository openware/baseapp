...
import { ieoFields, ieoTitle } from '../../constants';
...
class IEODetailsContainer extends React.Component<Props, State> {
    ...
    private renderContent = () => {
        ...
        const quoteCurrency = currencies.length && currentIEO && currentIEO.pairs[0].quote_currency_id && currencies.find(currency => (currency.id && currency.id.toLowerCase() === currentIEO.pairs[0].quote_currency_id.toLowerCase()) || false);
        const introduction = currentIEO && currentIEO.metadata && currentIEO.metadata.introduction;
        ...
        return (
            <React.Fragment>
                ...
                <div className="pg-currentIEO-page__product-intiduction">
                    {introduction && <IEOProjectIntroduction introduction={introduction} />}
                </div>
    ...
    private handleClickBack = () => {
        this.props.history.push(`/${ieoTitle.toLowerCase()}`);
    };
...
export const IEODetailsScreen = withRouter(injectIntl(connect(mapStateToProps, mapDispatchToProps)(IEODetailsContainer as any) as any) as any) as any;
