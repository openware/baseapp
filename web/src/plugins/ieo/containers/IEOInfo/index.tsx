...
type Props = OwnProps & IntlProps & RouterProps;
...
class IEOInfoComponent extends React.Component<Props, State> {
    ...
    public componentDidUpdate(prevProps: Props, prevState: State) {
        ...
        if (prevProps.ieo && ieo && prevProps.ieo.state !== ieo.state) {
            ...
            this.countdownInterval = setInterval(() => {
                if (ieo.state === 'distributing' && this.props.ieo.type === 'proportional') {
                    countdownDate = ieo.finishes_at;
                    this.setState({ countdownValue: getCountdownDate(countdownDate, '5m')});
                } else {
                    this.setState({ countdownValue: getCountdownDate(countdownDate)});
    ...
    public render() {
        ...
        const iconUrl = (ieoFields.metadata && ieo.metadata && ieo.metadata.icon_url) || (currency && currency.icon_url);
                    ...
                    <div className="ieo-profile-info__main__info">
                        <div className="ieo-profile-info__main__info__coin-area">
                            <div className="ieo-profile-info__main__info__coin-area__coin-logo">
                                {iconUrl ? <img src={iconUrl} alt="ieo-icon" /> : <CryptoIcon code={ieo.currency_id.toUpperCase()} />}
                            </div>
            ...
            <div className="ieo-profile-info__main__info__value">
                ...
                <div className="ieo-goal">
                    ...
                    <div className="ieo-goal__value">
                        {Decimal.format(ieo.supply, +((ieoFields.metadata && ieo.metadata && ieo.metadata.precision) || (currency && currency.precision) || 0))}&nbsp;
                        {ieo.currency_id && ieo.currency_id.toUpperCase()}
                    </div>
                </div>
            </div>
    ...
    private renderProgressBar = () => {
        const { ieo, currency } = this.props;
        const percentage = +ieo.supply ? +Decimal.format((+(ieo.tokens_ordered || 0) * 100) / +ieo.supply, 2) : 0;
        ...
        const precision = (ieoFields.metadata && ieo.metadata && ieo.metadata.precision) || (currency && currency.precision) || 0;

        return (
            <div className="curent-progress-block">
                <div className="curent-progress-block__price">
                    {Decimal.format(ieo.tokens_ordered, +precision)}&nbsp;/{Decimal.format(ieo.supply, +precision)}&nbsp;&nbsp;{ieo.currency_id && ieo.currency_id.toUpperCase()}
                </div>
