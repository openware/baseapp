...
export const TickerTable = (props: Props) => {
    const { currentBidUnit, markets, setCurrentBidUnit, currentBidUnitsList, redirectToTrading } = props;
    ...
    ), [formatMessage, currentBidUnit, currentBidUnitsList, setCurrentBidUnit]);

    const renderItem = React.useCallback((market, index: number) => {
                ...
                <table className="pg-ticker-table__table">
                    ...
                    <tbody>
                        {markets[0] ? (
                            ...
                            <tr><td><span className="no-data">{formatMessage({id: 'page.noDataToShow'})}</span></td></tr>
                        )}
                    </tbody>
                </table>
            </div>
