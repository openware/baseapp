...
class RecentTradesYoursContainer extends React.Component<Props> {
    ...
    private renderRow = (item, i) => {
        ...
        return [
            ...
            <span style={{ color: setTradesType(orderSide).color }} key={id}><Decimal key={id} fixed={amountFixed} thousSep=",">{amount}</Decimal></span>,
            <span style={{ color: setTradesType(orderSide).color }} key={id}><Decimal key={id} fixed={priceFixed} prevValue={[...list][i - 1] ? [...list][i - 1].price : 0} thousSep=",">{price}</Decimal></span>,
        ];
    };
